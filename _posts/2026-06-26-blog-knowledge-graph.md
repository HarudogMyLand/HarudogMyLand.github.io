...
博客首页加入了一个 Obsidian-like 的 Knowledge Graph，用节点图展示文章之间的关系。

我希望它（真的）能表达博客结构：哪些文章属于同一个专栏，哪些文章共享标签，哪些主题之间有联系。
...
这样做使得联系稳定。专栏可以手动维护，tag 可以保持灵活，图谱不会因为自动分类结果变化而频繁漂移。

## 渲染方式

使用原生 SVG 和少量 JavaScript。主要元素：
...
之后节点仍然参与力导向计算，但会倾向于留在用户拖动后的位置附近。这样既保留了图谱的动态性，也避免节点被立刻拉回原来的聚类中心。
