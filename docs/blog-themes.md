# Blog Theme Notes

博客首页 `/year-archive/` 现在有两套彼此独立的主题机制：

- 全站明暗色：由顶部导航的月亮/太阳按钮控制，落在 `html[data-theme="dark"]`。
- 博客排版主题：由 Blog 标题区域的 `Theme` 按钮控制，落在 `body[data-blog-theme="..."]`。

`Theme` 按钮只切换博客首页的 CSS 主题和排版方式，不负责明暗色。用户选择会写入 `localStorage.blogTheme`，下次进入博客页会自动恢复。

## Current Themes

### `source`

参考“芋道源码”这类古早源码解析博客：

- 技术索引感更强，适合代码笔记和教程目录。
- 主交互色为蓝色，避免荧光绿。
- 页面背景偏灰，内容区偏白。
- Knowledge Graph 和 Columns 更像条目入口，密度较高。

### `gov`

参考政府门户网站：

- 信息门户式排版，标题层级清楚。
- 蓝色为主色，红色只作为标题前置强调。
- 留白更正式，模块关系更稳。
- 适合公告、归档、长列表阅读。

### `marx`

参考马克思主义文库式资料站：

- 早期文献库/档案站的排版气质。
- 使用 Tahoma、Times New Roman、宋体一类字体组合。
- 海军蓝链接、浅青色内容块、红色 hover 强调。
- 更像资料目录和文献索引。

## Files

- `_pages/year-archive.html`
  - 博客首页结构。
  - 年/月/日热力图切换。
  - Knowledge Graph 节点关系图。
  - 标签筛选。
  - 独立专栏渲染。
  - 博客排版主题切换脚本。
- `_data/blog_columns.yml`
  - 专栏定义。
  - 专栏标题、描述、图标和稳定 key。
- `_sass/layout/_blog.scss`
  - 博客文章阅读页样式。
  - 博客首页共享样式。
  - 文章页浮动目录与专栏归属模块样式。
  - `body.blog-archive[data-blog-theme="..."]` 主题覆盖。
- `_layouts/archive.html`
  - 给博客首页增加 `blog-home-shell`。
  - 在 `blog_home: true` 时隐藏默认 archive 标题。
- `_layouts/default.html`
  - 给博客相关页面增加 `blog-post` / `blog-archive` body class。
  - 给博客首页提供默认 `data-blog-theme="source"`。

## Add A New Blog Theme

1. 在 `_pages/year-archive.html` 的 `blogThemes` 数组中增加一个主题：

```js
{ key: 'new-theme', label: 'New Theme' }
```

2. 在 `_sass/layout/_blog.scss` 中增加对应的作用域：

```scss
body.blog-archive[data-blog-theme="new-theme"] {
  --blog-home-bg: #ffffff;
  --blog-module-bg: #ffffff;
  --blog-text-muted: #666666;
  --blog-rule: #dddddd;
  --blog-accent: #005bac;
  --blog-accent-soft: rgba(0, 91, 172, 0.1);
  --blog-theme-font: Arial, sans-serif;
  --blog-theme-heading-font: Arial, sans-serif;
}
```

3. 只在同一个选择器作用域下添加组件覆盖，例如：

```scss
body.blog-archive[data-blog-theme="new-theme"] .blog-home__section-head {
  background: linear-gradient(to bottom, transparent calc(100% - 1px), #dddddd 0);
}
```

## Design Rules

- 新主题优先通过 CSS 变量、字体、色块、标题组织和间距建立差异。
- 尽量不要新增模块外框、圆角卡片边框或额外 wrapper。
- 博客排版主题不要写到 `html[data-theme="dark"]`，明暗色仍然是全站级功能。
- 新增主题时保持 `body.blog-archive[data-blog-theme="..."]` 作用域，避免影响文章正文页和其他页面。

## Columns

专栏和标签是两套不同系统：

- `tags`：用于 Posts 区域的标签筛选。
- `columns`：用于 Columns 区域的专栏收录。

一篇文章可以有 `Docker` 标签，但不一定进入 Docker 专栏；同样，一篇文章可以进入 `computer-organization` 专栏，而不需要拥有同名 tag。

### Add A Column

1. 在 `_data/blog_columns.yml` 中添加专栏：

```yml
- key: computer-organization
  title: Computer Organization
  description: Architecture, memory, operating systems, and low-level CS notes.
  icon: fas fa-microchip
```

2. 在文章 front matter 中显式收录：

```yml
columns:
  - computer-organization
```

3. 博客首页会自动把该文章收入对应专栏。点击专栏会跳转到该专栏的第一篇文章。

专栏 key 应保持稳定，建议只使用小写字母、数字和短横线。

## Knowledge Graph

博客首页的 Knowledge Graph 是一个 Obsidian-like 的节点图：

- 节点：每篇博客文章。
- 主聚类：优先使用文章 front matter 中的第一项 `columns`。
- fallback 聚类：如果文章没有 `columns`，使用第一项 `tags`。
- 连线：两篇文章共享 column 时连强边，共享 tag 时连弱边。
- 跳转：点击节点直接打开文章原文。

当前没有使用 NLP 自动分类。原因是博客专栏更接近编辑选择，而不是文本聚类结果；手动维护 `columns` 更稳定，也更符合“专栏是策划出来的阅读路径”这个目标。

布局规则：

- 节点会在有限时间内动态计算位置，稳定后自动停止，以减少 CPU 与电池消耗，并且可以被鼠标/触控拖动。
- 节点最终被限制在 SVG 画布边界内，不会跑出主屏幕。
- 同聚类节点会使用同一种主题色，并被轻微吸引到同一个聚类中心，但内部仍然保持斥力。
- 不同聚类之间排斥力更强，距离更大。
- 共享 column 的边比共享 tag 的边更短、更强。
- 标题默认显示；`Labels` switch 可收起标题，只在 hover/focus 时显示。
- 顶部标题区和图谱工具栏各有一个搜索框，二者同步输入。
- 搜索框可按标题、日期、tag、column 和聚类名高亮节点。
- 鼠标悬浮在图谱上时可以滚轮缩放，缩放会围绕鼠标位置进行；工具栏也提供缩放按钮。
- 鼠标或触控在空白区域拖动可以平移图谱视角，用于找回缩放后移出视野的节点。
- hover 到节点时，右侧 inspector 会显示该节点所属聚类名称。
- 工具栏提供暂停/恢复动画与重置视图按钮；系统启用“减少动态效果”时默认暂停。
- 图谱下方提供普通文章列表，作为键盘、辅助技术和低性能设备的替代入口。

后续如果文章数量明显增加，可以继续扩展：

- 按 column/tag 筛选图谱。
- 从文章正文链接中生成更高权重的边。
- 在缩放较小时隐藏部分标签，减少视觉拥挤。

## Post Page Extras

博客文章页有两个自动生成的组件：

- Floating TOC：右侧可折叠目录，自动扫描正文中的 `h2` 和 `h3`。
- Column Affiliation：文章底部专栏归属模块，读取文章 front matter 中的 `columns`。

文章页不需要手动维护目录。只要正文中有二级或三级标题，右侧目录就会自动生成；如果没有标题，目录会自动隐藏。

专栏归属模块会显示：

- `本文隶属于 XX 专栏`
- 专栏描述
- 该专栏下全部文章的纵向滚动列表

如果一篇文章属于多个专栏：

```yml
columns:
  - writing-lab
  - systems-tools
```

## Languages

顶部导航的 Language 控件会根据页面文件名自动寻找语言版本。

- 语言后缀放在扩展名之前，例如 `markdown-CN.md`、`markdown-EH.html`。
- 语言菜单扫描全站文件名中存在的显式大写后缀；不带后缀的旧页面按 `EH` 英文处理。
- 配对依据是去掉扩展名和最后语言后缀后的文件路径，因此 `.md`、`.html` 等扩展名不影响配对。
- 语言代码使用大写字母；当前使用 `EH` 表示英文、`CN` 表示中文，也支持其他大写语言代码。
- 新增翻译页面时，建议保持原文件所在目录、基础文件名和 permalink 结构一致。

例如：

```text
_pages/markdown-EH.md
_pages/markdown-CN.html
```

页面底部会分别显示多个专栏分组。
